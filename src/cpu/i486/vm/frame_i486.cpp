...
frame frame::sender_for_interpreter_frame(RegisterMap* map) const {
  ...
  // so this if EBP was live on callout from c2 we can find
  // the saved copy no matter what it called.
  // Since the interpreter saves EBP always if we record where it is then
  // we don't have to always save EBP on entry and exit to c2 compiled
