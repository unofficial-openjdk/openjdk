...
${TESTJAVA}/bin/javac -XX:AddModuleExports=jdk.jdi/com.sun.tools.example.debug.tty -d "${TESTCLASSES}" ${CP} -g \
                         "${TESTSRC}"/"${TARGETCLASS}".java
#
#Run the test class, again with the classpath we need:
${TESTJAVA}/bin/java -XX:AddModuleExports=jdk.jdi/com.sun.tools.example.debug.tty ${CP} ${TARGETCLASS}
status=$?
