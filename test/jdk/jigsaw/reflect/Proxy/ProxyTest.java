...
public class ProxyTest {
    ...
    public void compileAll() throws Exception {
        ...
        // compile ProxyAccess.java to access APIs in named modules
        Path source = SRC_DIR.resolve("ProxyAccess.java");
        assertTrue(CompilerUtils.compile(source, CPATH_DIR, "-mp", MODS_DIR.toString()));
    }
    ...
    /**
     * Run the ProxyAccess test with jdk.test.Main in unnamed module
     */
    @Test
    public void runProxyAccessTest() throws Exception {
        Path testClasses = MODS_DIR.resolve("test");
        String cpath = CPATH_DIR.toString() + File.pathSeparator + testClasses.toString();
        int exitValue = executeTestJava("-cp", cpath,
                                        "-mp", MODS_DIR.toString(),
                                        "-addmods", "m1,m2,m3",
                                        "ProxyAccess")
                .outputTo(System.out)
