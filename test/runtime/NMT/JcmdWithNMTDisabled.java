...
public class JcmdWithNMTDisabled {
  ...
  public static void main(String args[]) throws Exception {
    ...
    if (args.length > 0) {
      ...
      pb = ProcessTools.createJavaProcessBuilder("-Dtest.jdk=" + testjdkPath, "-XaddExports:java.management/sun.management", "JcmdWithNMTDisabled");
      output = new OutputAnalyzer(pb.start());
      output.shouldHaveExitValue(0);

      // Then run with explicitly disabling NMT, should not be any difference
      pb = ProcessTools.createJavaProcessBuilder("-Dtest.jdk=" + testjdkPath, "-XX:NativeMemoryTracking=off", "-XaddExports:java.management/sun.management", "JcmdWithNMTDisabled");
      output = new OutputAnalyzer(pb.start());
      output.shouldHaveExitValue(0);
